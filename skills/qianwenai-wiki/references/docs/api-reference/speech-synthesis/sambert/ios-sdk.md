...
本文档提供了语音合成 Sambert iOS SDK 的详细使用指南，帮助您将文本转换为高质量、富有表现力的语音。
...
1. **获取 API Key**：[获取 API Key](/api-reference/preparation/api-key)
   ...
   <Note>
     当需要为第三方应用或用户提供临时访问权限，或者希望严格控制敏感数据访问、删除等高风险操作时，建议使用[临时 API Key](/api-reference/more/generate-a-temporary-api-key)。临时 API Key 拥有固定的 60 秒有效期，过期后需重新获取。
   </Note>
...
1. 初始化 SDK。
2. 按业务需求设置参数：通过 [`nui_tts_initialize`](#nui-tts-initialize) 接口的 `parameters` 参数设置[连接与控制参数](#连接与控制参数)；通过 [`nui_tts_set_param`](#nui-tts-set-param) 接口设置[语音合成效果参数](#语音合成效果参数)。
3. 调用 [`nui_tts_play`](#nui-tts-play) 开始语音合成。
4. 在 [`onNuiTtsUserdataCallback`](#onnuittsuserdatacallback-监听音频数据和时间戳信息) 回调中获取音频数据，建议使用流式播放。如需保存本地，按追加模式将音频写入同一文件，直到合成完成。
5. 任务结束后，调用 [`nui_tts_release`](#nui-tts-release) 释放 SDK 资源。

## 请求参数

### 连接与控制参数

通过在 [`nui_tts_initialize`](#nui-tts-initialize) 接口的 `parameters` 参数中传入一个 JSON 字符串来配置。
...
| `debug_path`        | String | 否    | 日志文件的存储路径。此参数仅在调用 [`nui_tts_initialize`](#nui-tts-initialize) 接口时将 `save_log` 设为 `YES` 时生效。此时必须设置日志文件路径，否则将报错。本地最多保留两个日志文件。                                                                                                                                                                                                                                                                                                                                                                             |
| `max_log_file_size` | int    | 否    | 设定日志文件的最大字节数。此参数仅在调用 [`nui_tts_initialize`](#nui-tts-initialize) 接口时将 `save_log` 设为 `YES` 时生效。默认值：`104857600`（100 \* 1024 \* 1024 字节，即 100 MiB）。                                                                                                                                                                                                                                                                                                                                                        |
| `log_track_level`   | int    | 否    | 控制通过日志回调（[`onNuiTtsLogTrackCallback`](#onnuittslogtrackcallback-监听追踪日志)）对外发送的日志内容的过滤级别。默认值：`2`。取值范围：`0`（LOG\_LEVEL\_VERBOSE）、`1`（LOG\_LEVEL\_DEBUG）、`2`（LOG\_LEVEL\_INFO）、`3`（LOG\_LEVEL\_WARNING）、`4`（LOG\_LEVEL\_ERROR）、`5`（LOG\_LEVEL\_NONE，表示关闭此功能）。注意：`log_track_level` 与 `level`（通过 [`nui_tts_initialize`](#nui-tts-initialize) 接口设置）共同决定最终回调的日志。一条日志的级别数值必须同时大于或等于 `log_track_level` 和 `level` 的值，才会被回调。例如，`log_track_level` 设为 2（INFO），`level` 设为 3（WARNING），则只有 WARNING 及以上级别（数值 >=3）的日志才会被回调。 |

### 语音合成效果参数

通过 [`nui_tts_set_param`](#nui-tts-set-param) 接口进行设置。
...
| `sample_rate`               | String | 否    | 采样率（单位 Hz）。默认值：模型对应的默认采样率。推荐使用[模型](#模型列表)的默认值。若不匹配，服务端会进行重采样。                                                       |
...
初始化语音合成 SDK 实例。SDK 为单例模式，在调用 [`nui_tts_release`](#nui-tts-release) 前禁止重复初始化。
...
| 参数           | 类型               | 说明                                                                                           |
| ------------ | ---------------- | -------------------------------------------------------------------------------------------- |
| `parameters` | `char*`          | JSON 字符串，包含鉴权、连接和调试参数。参见[连接与控制参数](#连接与控制参数)。                                                 |
| `level`      | `NuiSdkLogLevel` | 控制 SDK 自身日志的打印级别。                                                                            |
| `save_log`   | `BOOL`           | 是否保存本地日志。若为 `YES`，须在[连接与控制参数](#连接与控制参数)通过 `debug_path` 指定路径，并可通过 `max_log_file_size` 设置文件大小。 |

**返回值说明**：返回错误码，参见[错误码查询](/api-reference/preparation/error-messages)。

#### nui\_tts\_set\_param

以键值对的形式设置[语音合成效果参数](#语音合成效果参数)。在 [`nui_tts_play`](#nui-tts-play) 之前调用。
...
| 参数      | 类型      | 说明                      |
| ------- | ------- | ----------------------- |
| `param` | `char*` | [语音合成效果参数名](#语音合成效果参数)。 |
| `value` | `char*` | [语音合成效果参数值](#语音合成效果参数)。 |
...
| 参数      | 类型      | 说明                     |
| ------- | ------- | ---------------------- |
| `param` | `char*` | 参数。目前仅支持 “error\_msg”。 |
...
| 参数         | 类型      | 说明                           |
| ---------- | ------- | ---------------------------- |
| `priority` | `char*` | 任务优先级。请将其设为 1。               |
| `taskid`   | `char*` | 任务 ID。传入 `null` 时由 SDK 自动生成。 |
| `text`     | `char*` | 待合成文本。                       |
...
暂停当前语音合成任务。任务暂停后，可通过 [`nui_tts_resume`](#nui-tts-resume) 恢复，或通过 [`nui_tts_cancel`](#nui-tts-cancel) 彻底取消。在任务暂停期间，SDK 不支持启动新的合成任务。
...
| 参数       | 类型      | 说明                                       |
| -------- | ------- | ---------------------------------------- |
| `taskid` | `char*` | 要取消的任务 ID。若传入 `null`，则取消所有正在暂停/进行中的合成任务。 |

**返回值说明**：返回错误码，参见[错误码查询](/api-reference/preparation/error-messages)。

#### nui\_tts\_release

释放 SDK 所有内部资源，并强制终止所有正在进行的合成任务。此方法调用后，SDK 实例将变为不可用状态，如需再次使用，必须重新调用 [`nui_tts_initialize`](#nui-tts-initialize) 进行初始化。
...
| 参数       | 类型               | 说明                                                                                   |
| -------- | ---------------- | ------------------------------------------------------------------------------------ |
| `event`  | `NuiSdkTtsEvent` | 回调事件。                                                                                |
| `taskid` | `char*`          | 语音合成任务 ID。                                                                           |
| `code`   | `int`            | 错误码，仅在事件 TTS\_EVENT\_ERROR 中有效。参见[错误码查询](/api-reference/preparation/error-messages)。 |
...
| 参数         | 类型      | 说明                                                                         |
| ---------- | ------- | -------------------------------------------------------------------------- |
| `info`     | `char*` | JSON 格式的时间戳结果。[语音合成效果参数](#语音合成效果参数) `word_timestamp_enabled` 设为 `"1"` 时生效。 |
| `info_len` | `int`   | `info` 字段的数据长度，可忽略。                                                        |
| `buffer`   | `char*` | 返回当前片段的音频数据。                                                               |
| `len`      | `int`   | 音频数据的长度（字节）。                                                               |
| `task_id`  | `char*` | 语音合成任务 ID。                                                                 |
...
| TTS\_EVENT\_ERROR  | 合成过程中发生错误。此时可通过 `nui_tts_get_param: "error_msg"` 获取详细错误信息。 |
