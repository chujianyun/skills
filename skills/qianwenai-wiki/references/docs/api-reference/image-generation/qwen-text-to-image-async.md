...
<Note>
  先[获取 API Key](/api-reference/preparation/api-key) 并[配置环境变量](/api-reference/preparation/export-api-key-env)。如果使用 SDK，还需[安装 SDK](/api-reference/preparation/install-sdk)。此异步接口仅支持 `qwen-image-plus` 和 `qwen-image` 模型，其他模型请使用[同步接口](/api-reference/image-generation/qwen-text-to-image)。
</Note>
...
paths:
  /services/aigc/text2image/image-synthesis:
    post:
      operationId: createQwenImageAsync
      summary: 创建图像生成任务
      description: 使用 Qwen-Image 创建异步图像生成任务。仅支持 `qwen-image-plus` 和 `qwen-image` 模型，qwen-image-3.0 系列请使用[异步生成图像（3.0系列）](#createQwenImage30Async)接口。
      parameters:
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/QwenImageAsyncRequest"
      responses:
        "200":
          description: 任务提交成功
          content:
      ...
      x-codeSamples:
        ...
        - lang: python
          ...
          source: |-
            ...
            prompt = ("Healing-style hand-drawn poster featuring three puppies "
                      "playing with a ball on lush green grass. The main title "
                      "'Come Play Ball!' is prominently displayed at the top.")
          ...
          source: |-
            ...
            public class QwenImageAsync {
              ...
              public static void main(String[] args) throws Exception {
                String prompt = "Healing-style hand-drawn poster featuring three puppies "
                  + "playing with a ball on lush green grass. The main title "
                  + "'Come Play Ball!' is prominently displayed at the top.";
