...
paths:
  /fine-tunes:
    post:
      operationId: createFaceChainFinetuneTask
      summary: 提交训练任务
      description: |-
        提交 FaceChain 人物形象训练任务。上传 1-10 张人像图片后，API 立即返回 `job_id`，训练在后台异步执行。通过 `GET /fine-tunes/{job_id}` 轮询训练状态。

        - QPS 限制：2 次/秒
        - 并发任务数限制：每个账号同时只能有 1 个训练任务在运行
      requestBody:
        required: true
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/FaceChainFinetuneRequest"
      responses:
        "200":
          ...
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/FaceChainFinetuneResponse"
              examples:
                success:
                  summary: 任务已接受（PENDING）
                  value:
              ...
              examples:
                ...
                UnsupportedOperation:
                  ...
                  value:
                    ...
                    message: The fine-tune job can not be deleted because it is succeeded,failed or canceled
        "401":
              ...
              examples:
                Conflict:
                  ...
                  value:
                    request_id: a4d78a5f-655f-9639-8437-xxxxxx
                    code: Conflict
                    message: Model instance xxxxx already exists, please specify a suffix
        "429":
          description: 请求频率超限
          content:
            application/json:
              ...
              examples:
                Throttling:
                  ...
                  value:
                    request_id: a4d78a5f-655f-9639-8437-xxxxxx
                    code: Throttling
                    message: Too many fine-tune job in running, please retry later
        "500":
          description: 服务器内部错误
          content:
            application/json:
