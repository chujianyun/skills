...
````yaml post /services/aigc/album/gen_potrait
openapi: 3.1.0
info:
...
paths:
  /services/aigc/album/gen_potrait:
    post:
      operationId: createFaceChainGenerationTask
      summary: 提交人物写真生成任务
      description: |-
        提交一个异步的人物写真生成任务。支持基于 LoRA 的预设风格写真、自定义模板写真，以及免训练（TrainFree）自定义模板写真。接口会立即返回 `task_id`，需通过 `GET /tasks/{task_id}` 轮询获取任务状态和结果。

        **输入限制**：
        - 人物单人照：JPEG/JPG/PNG 格式，不超过 3MB，分辨率 256×256 ~ 2048×2048，人脸占比不低于 128×128，清晰正脸（角度不超过 15 度），1~5 张
        - 自定义模板：JPEG/JPG/PNG 格式，不超过 5MB，分辨率 512×512 ~ 1680×1260，单人清晰风格模板图
        - URL 地址中不能包含中文字符

        **接口限制**：单账户（含主账号与 RAM 子账号）任务下发 QPS 为 2，并发任务数量限制为 1。
      parameters:
        - name: X-DashScope-Async
          in: header
          required: true
          description: 固定设置为 `enable`，表示使用异步方式提交任务。
          schema:
            type: string
            enum:
              - enable
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/FaceChainGenerationRequest"
      responses:
        "200":
          description: 任务提交成功
          content:
              ...
              examples:
                InvalidParameter:
                  ...
                  value:
                    ...
                    message: The request is missing required parameters or the parameters are out of the specified range, please check the parameters that you send
                InvalidURL:
                  summary: URL 无效
                  value:
                    request_id: fb53c4ec-1c12-4fc4-a580-cdb7c3261fc1
                    code: InvalidURL
                    message: The request URL is invalid, make sure the url is correct and is an image
                ...
                InvalidParameterDataInspection:
                  summary: 数据安全审查无法访问资源
                  value:
                    request_id: fb53c4ec-1c12-4fc4-a580-cdb7c3261fc1
                    code: InvalidParameter.DataInspection
                    message: Don't have authorization to access the media resource during the data inspection process.
                BadRequestEmptyModel:
                  summary: 缺少必选参数 model
                  value:
                    request_id: fb53c4ec-1c12-4fc4-a580-cdb7c3261fc1
            ...
            application/json:
              ...
              examples:
                ResourceAccessDenied:
                  ...
                  value:
                    request_id: fb53c4ec-1c12-4fc4-a580-cdb7c3261fc1
                    code: Resource.AccessDenied
                    message: Resource {resource_id} access denied.
        "500":
          description: 内部服务错误
          content:
            application/json:
              ...
              examples:
                InternalErrorAlgo:
                  ...
                  value:
                    ...
                    message: An internal error occurs during computation, please try this model later
                InternalErrorInference:
                  summary: 推理错误
                  value:
                    request_id: fb53c4ec-1c12-4fc4-a580-cdb7c3261fc1
                    code: InternalError.Algo
                    message: "Inference error: Inference error"
      x-codeSamples:
        - lang: curl
          label: 预设风格（LoRA 模式）
          source: |-
        ...
        - lang: curl
          ...
          source: |-
            ...
            --data '{
              ...
              "input": {
                "template_url": "https://example.com/template.jpg",
                "user_urls": ["https://example.com/face.jpg"]
              }
            }'
components:
  securitySchemes:
