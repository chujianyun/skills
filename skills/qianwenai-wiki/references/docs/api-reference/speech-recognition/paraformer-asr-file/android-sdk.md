...
| `nls_config.vocabulary_id`               | string          | 否    | 热词词表ID，用于提升特定词汇识别准确率。适用于v2及以上版本模型，参见[定制热词](/developer-guides/speech/improve-recognition-accuracy)。                                                            |
| `nls_config.resources`                   | array\[object]  | 否    | v1版本模型热词配置，每个元素包含 `resource_id`（热词ID）和 `resource_type`（固定为 `"asr_phrase"`），参见[Paraformer热词定制与管理](/api-reference/speech-recognition/custom-hotwords/http-api)。 |
