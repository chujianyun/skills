...
> 查询 HappyHorse 视频编辑任务状态

轮询任务状态，任务成功后下载视频。

## 轮询策略

1. 保存[提交任务](/api-reference/video-generation/happyhorse-video-editing/create-task)响应中的 `task_id`。
...
paths:
  /tasks/{task_id}:
    get:
      operationId: getHappyHorseVideoEditingTaskStatus
      summary: 查询视频编辑任务结果
      ...
      responses:
        "200":
          ...
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/HappyHorseVideoEditTaskStatusResponse"
              examples:
                SUCCEEDED:
                  summary: 任务执行成功
                  value:
