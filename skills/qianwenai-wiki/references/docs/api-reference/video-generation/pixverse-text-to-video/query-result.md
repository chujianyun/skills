...
# 爱诗 PixVerse — 查询文生视频任务结果

> 查询爱诗 PixVerse 文生视频任务状态，获取生成的视频

查询任务状态并获取生成的视频。

## 轮询策略

1. 调用[提交文生视频任务](/api-reference/video-generation/pixverse-text-to-video/create-task)接口获取 `task_id`。
...
paths:
  /tasks/{task_id}:
    get:
      operationId: getPixVerseTextToVideoTaskStatus
      summary: 查询任务结果
      ...
      responses:
        "200":
          ...
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/PixVerseT2VTaskStatusResponse"
              examples:
                SUCCEEDED:
                  summary: 任务执行成功
                  value:
                    ...
                    output:
                      ...
                      orig_prompt: 下着雨，赛博城市里，一只浣熊在栏杆上行走。
