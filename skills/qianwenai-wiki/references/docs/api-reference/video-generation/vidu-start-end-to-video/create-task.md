...
> 提交首尾帧生视频任务，使用 Vidu 模型基于首帧和尾帧图像生成过渡视频。

以首帧图像和尾帧图像为输入，由模型插值生成两帧之间的过渡视频。
...
- **宽高比**：1:4 至 4:1
- **文件大小**：每张图像不超过 50 MB
- **帧一致性**：首帧和尾帧的总像素数（宽×高）比值需在 0.8～1.25 之间
...
1. 记录响应中的 `task_id`。
2. 每隔 **15 秒**调用[查询视频结果](/api-reference/video-generation/vidu-start-end-to-video/query-result)接口轮询。
3. 当 `task_status` 为 `SUCCEEDED` 时，从 `output.video_url` 下载生成的视频。
...
paths:
  /services/aigc/video-generation/video-synthesis:
    post:
      operationId: createViduStartEndToVideoTask
      summary: 创建首尾帧生视频任务
      description: 提交首尾帧生视频任务，以首帧图像和尾帧图像为输入，生成两帧之间平滑过渡的视频。
      parameters:
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/ViduStartEndToVideoRequest"
      responses:
        "200":
          description: 任务提交成功
          content:
