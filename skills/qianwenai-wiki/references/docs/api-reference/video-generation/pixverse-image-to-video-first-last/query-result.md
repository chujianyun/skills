...
> 查询 PixVerse 首尾帧图生视频任务状态

查询任务状态并获取生成的视频。

## 轮询策略

1. 通过[创建任务](/api-reference/video-generation/pixverse-image-to-video-first-last/create-task)接口提交任务，保存返回的 `task_id`。
2. 每 **15 秒**轮询一次，直到 `task_status` 变为 `SUCCEEDED` 或 `FAILED`。
3. 任务成功后，从 `video_url` 下载视频。

## 注意事项

- **URL 有效期**：`video_url` 当前无过期时间限制，但不建议将其作为长期存储，请及时下载。
...
paths:
  /tasks/{task_id}:
    get:
      operationId: getPixVerseKf2vTaskStatus
      summary: 查询任务结果
      description: 根据 task_id 查询首尾帧生视频任务的状态和结果。task_id 有效期为 24 小时，超时后将无法查询。建议采用轮询机制，设置合理的查询间隔（如 15 秒）来获取结果。
      parameters:
        - name: task_id
          in: path
          required: true
          description: 任务ID。将 {task_id} 完整替换为创建任务接口返回的 task_id 的值。
          schema:
            type: string
      responses:
        "200":
            ...
            application/json:
              ...
              examples:
                ...
                FAILED:
                  summary: 任务执行失败
                  value:
                    request_id: e5d70b02-xxxx-xxxx-xxxx-xxxxxx
                    output:
                      task_id: 86ecf553-xxxx-xxxx-xxxx-xxxxxx
                      task_status: FAILED
                ...
                RUNNING:
                  summary: 任务处理中
                  value:
                    request_id: c2f81a10-xxxx-xxxx-xxxx-xxxxxx
                    output:
                      task_id: 3d7b9e20-xxxx-xxxx-xxxx-xxxxxx
                      task_status: RUNNING
                      submit_time: 2026-03-20 13:47:42.916
                      scheduled_time: 2026-03-20 13:47:42.949
                UNKNOWN:
                  summary: 任务查询过期
                  value:
                    request_id: a4de7c32-xxxx-xxxx-xxxx-xxxxxx
                    output:
                      task_id: 502a00b1-xxxx-xxxx-xxxx-xxxxxx
                      task_status: UNKNOWN
        "400":
          description: 请求参数错误
          content:
