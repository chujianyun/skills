...
以上功能可通过自定义Prompt实现，具体用法请参见[功能示例](#功能示例)。
...
更多信息请参见[模型列表](/developer-guides/getting-started/model-selection)。
...
**前提条件**：已[获取 API Key](/api-reference/preparation/api-key)。如果通过SDK调用，需要安装最新版SDK。
...
以下每个功能示例提供可直接运行的 `content`，替换[代码示例](#代码示例)中的 `content` 即可体验。

<Accordion title="对话信息抽取">
