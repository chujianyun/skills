# developers.llamaindex.ai/python/framework/module_guides/loading/node_parsers/index.md
> https://developers.llamaindex.ai/python/framework/module_guides/loading/node_parsers/index.md

--- title: Node Parser Usage Pattern | Developer Documentation ---

Node parsers are a simple abstraction that take a list of documents, and chunk them into `Node` objects, such that each node is a specific chunk of the parent document. When a document is broken into nodes, all of it’s attributes are inherited to the children nodes (i.e. `metadata`, text and metadata templates, etc.). You can read more about `Node` and `Document` properties [here] (/python/framework/module_guides/loading/documents_and_nodes/index.md).
...
# per-index index = VectorStoreIndex.from_documents( documents, transformations=[SentenceSplitter(chunk_size=1024, chunk_overlap=20)], ) ```
