# developers.llamaindex.ai/python/framework/understanding/agent/multi_agent/index.md
> https://developers.llamaindex.ai/python/framework/understanding/agent/multi_agent/index.md

--- title: Multi-agent patterns in LlamaIndex | Developer Documentation ---
...
1. **AgentWorkflow (built-in)** – declare a set of agents and let `AgentWorkflow` manage the hand-offs. Section [Full Notebook] (/python/examples/agent/agent_workflow_multi/index.md) 2. **Orchestrator pattern (built-in)** – an “orchestrator” agent chooses which sub-agent to call next; those sub-agents are exposed to it as **tools**. Section Full Notebook 3. **Custom planner (DIY)** – you write the LLM prompt (often XML / JSON) that plans the sequence yourself and imperatively invoke the agents in code. Section Full Notebook
...
`AgentWorkflow` is itself a Workflow pre-configured to understand agents, state and tool-calling. You supply an *array* of one or more agents, tell it which one should start, and it will:
1. Give the *root* agent the user message. 2. Execute whatever tools that agent selects. 3. Allow the agent to “handoff” control to another agent when it decides. 4. Repeat until an agent returns a final answer.
**NOTE:** At any point, the current active agent can choose to return control back to the user.
Below is the condensed version of the multi-agent report generation example. Three agents collaborate to research, write and review a report. (`…` indicates code omitted for brevity.)
...
resp = await agent_workflow.run( user_msg="Write me a report on the history of the web …" ) print(resp) ```
...
You can see the full example in the agents\_as\_tools notebook.
``` import re from llama_index.core.agent.workflow import FunctionAgent from llama_index.core.workflow import Context
...
response = await orchestrator.run( user_msg="Write me a report on the history of the web …" ) print(response) ```
...
Below is a *minimal* sketch of a workflow that can plan, execute a plan, and see if any further steps are needed. You can see the full example in the [custom\_multi\_agent notebook] (/python/examples/agent/custom_multi_agent/index.md).
``` import re import xml.etree.ElementTree as ET from pydantic import BaseModel, Field from typing import Any, Optional

from llama_index.core.llms import ChatMessage from llama_index.core.workflow import ( Context, Event, StartEvent, StopEvent, Workflow, step, )
...
# Modelling the plan class PlanStep(BaseModel): agent_name: str agent_input: str
...
---
## Choosing a pattern
| Pattern | Lines of code | Flexibility | Built-in streaming / events | | ------------------ | ------------- | ----------- | --------------------------------------------- | | AgentWorkflow | ⭐ – fewest | ★★ | Yes | | Orchestrator agent | ⭐⭐ | ★★★ | Yes (via orchestrator) | | Custom planner | ⭐⭐⭐ | ★★★★★ | Yes (via sub-agents). Top-level is up to you.
...
Next you will learn how to use structured output in single and multi-agent workflows