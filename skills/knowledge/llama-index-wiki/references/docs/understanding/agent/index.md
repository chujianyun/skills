# developers.llamaindex.ai/python/framework/understanding/agent/index.md
> https://developers.llamaindex.ai/python/framework/understanding/agent/index.md

--- title: Building an agent | Developer Documentation ---
...
To learn about the various ways to build multi-agent systems, go to “Multi-agent systems”.
agent flow
## Getting started
You can find all of this code in the agents tutorial repo [1].
...
## OpenAI Key
Our agent will be powered by OpenAI’s `gpt-4o-mini` LLM, so you’ll need an API key [2]. Once you have your key, you can put it in a `.env` file in the root of your project:
Terminal window
...
from llama_index.llms.openai import OpenAI from llama_index.core.agent.workflow import FunctionAgent ```
## Create basic tools
...
You could also pick another popular model accessible via API, such as those from Mistral, Claude from Anthropic or [Gemini from Google] (/python/examples/llm/google_genai/index.md).
...
``` workflow = FunctionAgent( tools=[multiply, add], llm=llm, system_prompt="You are an agent that can perform basic mathematical operations using tools.", ) ```
...
``` response = await workflow.run(user_msg="What is 20+(2*4)?") print(response) ```
...
Check the repo [3] to see what the final code should look like.
Congratulations! You’ve built the most basic kind of agent. Next let’s learn how to use pre-built tools.

[1] https://github.com/run-llama/python-agents-tutorial
[2] https://platform.openai.com/
[3] https://github.com/run-llama/python-agents-tutorial/blob/main/1_basic_agent.py