...
openapi: 3.1.0
info:
  title: CosyVoice 音色管理 API — 查询音色详情
  version: 1.0.0
servers:
...
paths:
  /services/audio/tts/customization:
    post:
      operationId: queryVoiceCosyVoice
      summary: 查询音色详情（CosyVoice）
      description: 查询指定 CosyVoice 音色的详情，包括状态、创建时间和原始音频 URL。
      requestBody:
        required: true
        content:
          application/json:
      ...
      properties:
        ...
        input:
          ...
          properties:
            action:
              type: string
              description: 固定为 `query_voice`。
              enum:
                - query_voice
              example: query_voice
            voice_id:
              type: string
              description: 要查询的音色 ID。
      ...
      properties:
        output:
          ...
          properties:
            ...
            resource_link:
              type: string
              description: 用于克隆的原始音频文件 URL。
              example: https://yourAudioFileUrl
            target_model:
              type: string
              description: 绑定到此音色的语音合成模型。
              example: cosyvoice-v3-plus
            status:
              type: string
              description: 音色状态：`DEPLOYING`（审核中）、`OK`（已通过，可使用）或 `UNDEPLOYED`（未通过）。
              enum:
                - DEPLOYING
                - OK
                - UNDEPLOYED
