...
使用 qwen3-rerank 模型根据语义相关性对文档进行重排序。采用嵌套请求结构，通过 `input` 和 `parameters` 组织请求参数。
...
````yaml post /services/rerank/text-rerank/text-rerank
openapi: 3.1.0
info:
...
paths:
  /services/rerank/text-rerank/text-rerank:
    post:
      operationId: createDashScopeRerank
      summary: DashScope
      description: 使用 qwen3-vl-rerank 或 gte-rerank-v2 对文档按语义相关度重新排序。采用嵌套请求结构，查询和文档封装在 `input` 对象中，配置参数封装在 `parameters` 对象中。
      servers:
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/DashScopeRerankRequest"
      responses:
        "200":
          description: 重排序结果
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/DashScopeRerankResponse"
        "400":
          description: 请求参数无效
          content:
            application/json:
          ...
          source: |-
            curl --location 'https://dashscope.aliyuncs.com/api/v1/services/rerank/text-rerank/text-rerank' \
              ...
              "input": {
                ...
                "documents": [
                  {"text": "Rerank models are widely used in search engines and recommendation systems."},
          ...
          source: |-
            curl --location 'https://dashscope.aliyuncs.com/api/v1/services/rerank/text-rerank/text-rerank' \
              ...
              "input": {
                ...
                "documents": [
                  "Rerank models are widely used in search engines and recommendation systems.",
          ...
          source: |-
            ...
            def vl_rerank():
              resp = dashscope.TextReRank.call(
                ...
                query={"image": "https://img.alicdn.com/imgextra/i3/O1CN01rdstgY1uiZWt8gqSL_!!6000000006071-0-tps-1970-356.jpg"},
                documents=[
                  {"text": "Rerank models are widely used in search engines."},
                  {"image": "https://img.alicdn.com/imgextra/i3/O1CN01rdstgY1uiZWt8gqSL_!!6000000006071-0-tps-1970-356.jpg"},
                  {"video": "https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20250107/lbcemt/new+video.mp4"}
                ],
                top_n=2,
