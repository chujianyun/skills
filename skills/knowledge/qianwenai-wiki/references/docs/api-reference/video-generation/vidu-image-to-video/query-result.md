...
> 查询 Vidu 图生视频任务的状态和结果。

在通过[生成视频](/api-reference/video-generation/vidu-image-to-video/create-task)接口提交任务后，轮询此接口获取结果。

## 轮询策略

1. 调用[生成视频](/api-reference/video-generation/vidu-image-to-video/create-task)接口获取 `task_id`。
2. 每隔 **15 秒**轮询，直到 `task_status` 变为 `SUCCEEDED` 或 `FAILED`。
3. 成功后，从 `output.video_url` 下载视频。

## 注意事项

- **URL 有效期**：视频 URL 自任务完成起 **24 小时**内有效，请及时下载。
- **状态转换**：`PENDING` → `RUNNING` → `SUCCEEDED` 或 `FAILED`。`UNKNOWN` 表示任务不存在或已过期。
...
paths:
  /tasks/{task_id}:
    get:
      operationId: getViduImageToVideoTaskStatus
      summary: 查询视频结果
      description: 查询 Vidu 图生视频任务的状态和结果。
      parameters:
        - name: task_id
          in: path
          required: true
