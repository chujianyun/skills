...
1. 通过[创建任务](/api-reference/video-generation/wan-video-character-swap/create-task)接口提交任务，保存返回的 `task_id`。
...
- **URL 有效期**：`video_url` 在 24 小时后过期，请及时下载。
...
paths:
  /tasks/{task_id}:
    get:
      operationId: getCharacterSwapTaskStatus
      summary: 查询任务结果
      description: 查询换脸任务的状态和结果。
      parameters:
        - name: task_id
          in: path
          required: true
