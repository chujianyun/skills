...
从单张图片生成最长 15 秒、最高 1080P 分辨率的视频，支持多镜头和自动音效。

## 模型选择

- **pixverse/pixverse-c1-it2v**：擅长动作、特效和快速运动场景。多镜头通过提示词格式实现（`镜头1：... 镜头2：...`），无需 `shot_type` 参数。
- **pixverse/pixverse-v6-it2v**：通用图生视频模型。多镜头需设置 `shot_type: "multi"`。
...
paths:
  /services/aigc/video-generation/video-synthesis:
    post:
      operationId: createPixVerseImageToVideo
      summary: 创建图生视频任务
      description: 提交图生视频异步任务，返回 task_id 用于后续轮询查询。task_id 有效期为 24 小时，请勿重复创建任务。
      parameters:
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/PixVerseImageToVideoRequest"
      responses:
        "200":
          description: 任务创建成功
          content:
      ...
      x-codeSamples:
        ...
        - lang: curl
          label: cURL - 图生视频多镜头（c1，prompt 方式）
          source: |-
            curl --location 'https://dashscope.aliyuncs.com/api/v1/services/aigc/video-generation/video-synthesis' \
              -H 'X-DashScope-Async: enable' \
              -H "Authorization: Bearer $DASHSCOPE_API_KEY" \
        ...
        - lang: curl
          label: cURL - 图生视频多镜头（v6，shot_type 参数）
          source: |-
            curl --location 'https://dashscope.aliyuncs.com/api/v1/services/aigc/video-generation/video-synthesis' \
              -H 'X-DashScope-Async: enable' \
              -H "Authorization: Bearer $DASHSCOPE_API_KEY" \
