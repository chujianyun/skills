...
paths:
  /tasks/{task_id}:
    get:
      operationId: getImageTranslationTaskStatus
      summary: 根据任务 ID 查询结果
      description: |-
        查询图像翻译任务的状态和结果。

        注意事项：
        - `task_id` 有效期为 **24 小时**，若 ID 不存在或已过期，任务状态将返回 `UNKNOWN`。
        - 任务成功后返回的 `image_url` 有效期为 **24 小时**，请及时下载并保存图像。
      parameters:
        - name: task_id
          in: path
          required: true
          description: 任务标识。由创建接口返回的 task_id。
          schema:
            type: string
      responses:
        "200":
          ...
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/ImageTranslationTaskStatusResponse"
              examples:
                SUCCEEDED:
                  summary: 任务执行成功（存在可翻译内容）
                  value:
      ...
      x-codeSamples:
        - lang: curl
          label: cURL - 查询任务结果
          source: |-
            # 将 86ecf553-d340-4e21-xxxxxxxxx 替换为真实的 task_id
