...
paths:
  /api/v1/services/aigc/multimodal-generation/generation:
    post:
      operationId: asrDashScopeSync
      summary: 识别音频（DashScope 同步）
      description: 通过 DashScope 同步 API 识别音频文件，支持 Qwen3-ASR-Flash。接受 URL、Base64 编码数据或本地文件路径（仅限 SDK）格式的音频输入。
      security:
        - bearer: []
      requestBody:
        required: true
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/DashScopeASRRequest"
      responses:
        "200":
          description: 成功响应
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/DashScopeASRResponse"
      x-codeSamples:
        - lang: bash
          label: cURL
          source: |-
          ...
          source: |-
            ...
            public class Main {
              public static void simpleMultiModalConversationCall()
                ...
                // asrOptions.put("language", "zh"); // 可选。若已知音频语言，可指定此参数以提高识别准确率。
        ...
        - lang: python
          ...
          source: |-
            ...
            messages = [
              {"role": "system", "content": [{"text": ""}]},  # 在此处配置自定义识别的上下文
            ...
            response = dashscope.MultiModalConversation.call(
              ...
              asr_options={
                #"language": "zh", # 可选。若已知音频语言，可指定此参数以提高识别准确率。
                "enable_itn":False
              }
            )
            print(response)
