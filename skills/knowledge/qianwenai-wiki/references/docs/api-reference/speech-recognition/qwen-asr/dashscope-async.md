...
1. **提交任务**：发送请求，服务端返回 `task_id`。
2. **获取结果**：使用 `task_id` 轮询[结果查询接口](/api-reference/speech-recognition/qwen-asr/query-result)，直到任务完成。

### SDK

1. 调用 `async_call()`（Python）或 `asyncCall()`（Java）提交任务，返回包含 `task_id` 的任务对象。
2. 使用任务对象或 `task_id` 调用 `fetch()`，SDK 会自动轮询直到任务完成或超时。

### REST API

1. 调用此接口提交任务，获取 `task_id`。
2. 使用 `task_id` [查询结果](/api-reference/speech-recognition/qwen-asr/query-result)。

## 音频格式

音频文件 URL 必须可公开访问。

`channel_id` 中的每个音频轨道单独计费。例如，单个文件指定 `[0, 1]` 会产生两次计费。

## OpenAPI

````yaml post /api/v1/services/audio/asr/transcription
openapi: 3.1.0
info:
...
paths:
  /api/v1/services/audio/asr/transcription:
    post:
      operationId: asrDashScopeAsync
      summary: 创建转录任务
      description: 创建异步音频转录任务。
      security:
        - bearer: []
      parameters:
        - name: X-DashScope-Async
          in: header
          required: true
          description: 必须设置为 `enable`，用于异步任务提交。
          schema:
            type: string
            enum:
              - enable
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/AsyncASRRequest"
      responses:
        "200":
          description: 任务创建成功
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/AsyncASRSubmitResponse"
      x-codeSamples:
        - lang: bash
          label: cURL
          source: |-
              ...
              public static void main(String[] args) {
                ...
                try (Response response = client.newCall(request).execute()) {
                  if (response.isSuccessful() && response.body() != null) {
                    ...
                    System.out.println("任务失败！HTTP 状态码：" + response.code());
