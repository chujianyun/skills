...
````yaml delete /compatible-mode/v1/responses/{response_id}
openapi: 3.1.0
info:
...
paths:
  /compatible-mode/v1/responses/{response_id}:
    delete:
      ...
      summary: 删除响应
      description: 根据 Response ID 删除一个已存储的模型响应。仅当原创建请求中 `store=true` 时返回的 Response ID 可被删除。
      parameters:
        - name: response_id
          in: path
          required: true
          schema:
            type: string
          description: 要删除的 Response ID，格式为 `resp_xxx`。仅当原创建请求中 `store=true` 时返回的 Response ID 可被删除。
      responses:
        "200":
          description: 删除成功
          content:
