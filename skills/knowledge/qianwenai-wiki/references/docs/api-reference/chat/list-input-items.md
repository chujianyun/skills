...
````yaml get /compatible-mode/v1/responses/{response_id}/input_items
openapi: 3.1.0
info:
...
security:
  - BearerAuth: []
paths:
  /compatible-mode/v1/responses/{response_id}/input_items:
    get:
      operationId: listInputItems
      summary: 获取输入项列表
      description: 获取生成指定 Response 时所使用的输入项列表。在多轮对话（使用 `previous_response_id` 串联）中，会一并返回历史轮次的用户输入和模型回复。仅当原创建请求中 `store=true` 时，返回的 Response ID 才支持查询。
      parameters:
        - name: response_id
          in: path
          required: true
          schema:
            type: string
          description: 要查询输入项的 Response ID，格式为 `resp_xxx`。仅当原创建请求中 `store=true` 时，返回的 Response ID 才支持查询。
        - name: after
          in: query
          required: false
          schema:
            type: string
          description: 以指定的 item ID（格式为 `msg_xxx`）作为起点，返回排在该 ID 之后的数据，顺序由 `order` 决定。item ID 来源：创建响应返回的 `output[].id`；或本接口返回的 `data[].id`、`first_id`、`last_id`。常用于翻页：把上一次返回的 `last_id` 作为本次的 `after` 传入，即可继续获取后续数据。
        - name: limit
          in: query
          required: false
          schema:
            ...
            maximum: 100
            default: 20
          description: 返回的最大条数，取值范围 [1, 100]，默认值为 20。
        - name: order
          in: query
          required: false
          schema:
            ...
            enum:
              ...
              - desc
            default: desc
          description: 排序方式，支持 `asc`（升序）和 `desc`（降序），默认值为 `desc`。
      responses:
        "200":
          description: 请求成功
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/InputItemsList"
              example:
                ...
                model: qwen-plus
        "404":
          description: 指定的 Response ID 不存在
          content:
            application/json:
              schema:
