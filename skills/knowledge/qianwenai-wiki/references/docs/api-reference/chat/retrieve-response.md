...
````yaml get /compatible-mode/v1/responses/{response_id}
openapi: 3.1.0
info:
...
security:
  - BearerAuth: []
paths:
  /compatible-mode/v1/responses/{response_id}:
    get:
      operationId: retrieveResponse
      summary: 获取响应
      description: 根据 Response ID 获取一个已完成的模型响应。仅当原创建请求中 `store=true` 时返回的 Response ID 可被检索。
      parameters:
        - name: response_id
          in: path
          required: true
          schema:
            type: string
          description: 要获取的 Response ID，格式为 `resp_xxx`。可从创建响应接口的响应中获取。仅当原创建请求中 `store=true` 时返回的 Response ID 可被检索。
      responses:
        "200":
          description: 请求成功
          content:
