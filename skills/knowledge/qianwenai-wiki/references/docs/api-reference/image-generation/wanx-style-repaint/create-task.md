...
将人像照片转换为多种艺术风格，同时保留面部特征和构图。支持预设风格和自定义参考图风格。

本 API 仅支持异步调用模式，暂不提供 SDK。
...
paths:
  /services/aigc/image-generation/generation:
    post:
      operationId: createWanxStyleRepaint
      summary: 创建人像风格重绘任务
      description: 创建人像风格重绘任务，获取唯一任务ID（task_id）。本模型采用异步模式，提交任务后需轮询获取结果。
      parameters:
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/WanxStyleRepaintRequest"
      responses:
        "200":
          description: 任务提交成功。请保存 task_id，用于查询任务状态与结果。
          content:
            application/json:
              schema:
