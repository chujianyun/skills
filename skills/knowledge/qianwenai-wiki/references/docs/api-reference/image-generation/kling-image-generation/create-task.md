...
> 提交可灵图像生成任务（文生图或参考图生图），获取 task_id 用于后续查询。
...
paths:
  /services/aigc/image-generation/generation:
    post:
      summary: 创建图像生成任务
      description: 提交可灵图像生成任务。图像生成任务有一定耗时（通常为1-2分钟），API采用异步调用。提交后请保存 task_id，用于查询任务状态与结果。
      operationId: createKlingImageGenerationTask
      parameters:
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/KlingImageGenerationRequest"
      responses:
        "200":
          description: 任务创建成功
          content:
