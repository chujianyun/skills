...
Harness 的 Cordis 事件遵循 `namespace/action` 命名，例如 `agent/pre-step`、`agent/request`、`agent/request-error`、`tools/result` 和 `session/event`。完整签名与触发模式见[子系统页面](https://deepseek-harness.github.io/deepseek-harness/reference/subsystems/core)上生成的 `cordis-surface` 区块。
...
- [能力分层](../实战/index.md)— 了解能力接口中的事件

- [LLM（大语言模型）适配器](../实战/LLM适配器.md)— 实现一个完整的 LLM 后端
