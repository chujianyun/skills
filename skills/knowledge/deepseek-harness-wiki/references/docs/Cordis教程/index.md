...
如果你想阅读精简的概念参考，而不是逐步实践，请参阅 [Cordis 入门](https://deepseek-harness.github.io/deepseek-harness/reference/cordis-primer)。详尽的 API 参考见[子系统页面](https://deepseek-harness.github.io/deepseek-harness/reference/subsystems/core)上生成的 `cordis-surface` 区块，以及 [Cordis 核心 API](https://deepseek-harness.github.io/deepseek-harness/reference/cordis-api/context) 页面。

如果你要为 harness 本身编写插件——由 `cordis.yml` 加载、在 Web UI 中驱动，而不是下面这个启动器——请从[第一个 Harness 插件](../基础/index.md)开始。
...
1. [你的第一个插件](01-第一个插件.md)：插件是函数，由 loader 挂载。

2. [生命周期与 effect](02-生命周期与副作用.md)：由 Cordis 管理的注册会在所属插件卸载时撤销。
...
6. [组合与 HMR（热模块替换）](06-组合与热重载.md)：把配置文件作为插件树，使用热重载，并诊断始终无法加载的插件。
