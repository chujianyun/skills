# developers.llamaindex.ai/python/framework/community/integrations/lmformatenforcer/index.md
> https://developers.llamaindex.ai/python/framework/community/integrations/lmformatenforcer/index.md

--- title: LM Format Enforcer | Developer Documentation ---
LM Format Enforcer [1] is a library that enforces the output format (JSON Schema, Regex etc) of a language model. Instead of just “suggesting” the desired output structure to the LLM, LM Format Enforcer can actually “force” the LLM output to follow the desired schema.
[2]
LM Format Enforcer works with local LLMs (currently supports `LlamaCPP` and `HuggingfaceLLM` backends), and operates only by processing the output logits of the LLM. This enables it to support advanced generation methods like beam search and batching, unlike other solutions that modify the generation loop itself. See the comparison table in the LM Format Enforcer page [1] for more details.
## JSON Schema Output
...
llm = LlamaCPP() regex_parser = lmformatenforcer.RegexParser(regex) lm_format_enforcer_fn = build_lm_format_enforcer_function(llm, regex_parser) with activate_lm_format_enforcer(llm, lm_format_enforcer_fn): output = llm.complete(prompt) ```
...
[2] https://raw.githubusercontent.com/noamgat/lm-format-enforcer/main/docs/Intro.webp