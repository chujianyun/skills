# developers.llamaindex.ai/python/framework/understanding/rag/loading/index.md
> https://developers.llamaindex.ai/python/framework/understanding/rag/loading/index.md

--- title: Loading Data (Ingestion) | Developer Documentation ---
...
1. Load the data 2. Transform the data 3. Index and store the data
...
In this example LlamaIndex downloads and installs the connector called DatabaseReader [1], which runs a query against a SQL database and returns every row of the results as a `Document`:
...
There are hundreds of connectors to use on LlamaHub [2]!
...
You can do this by either using our transformation modules (text splitters, metadata extractors, etc.) as standalone components, or compose them in our declarative [Transformation Pipeline interface] (/python/framework/module_guides/loading/ingestion_pipeline/index.md).
...
### Adding Metadata
You can also choose to add metadata to your documents and nodes. This can be done either manually or with [automatic metadata extractors] (/python/framework/module_guides/loading/documents_and_nodes/usage_metadata_extractor/index.md).
Here are guides on 1) how to customize Documents, and 2) [how to customize Nodes] (/python/framework/module_guides/loading/documents_and_nodes/usage_nodes/index.md).
``` document = Document( text="text", metadata={"filename": "<doc_file_name>", "category": "<category>"}, ) ```

### Adding Embeddings
To insert a node into a vector index, it should have an embedding. See our ingestion pipeline or our [embeddings guide] (/python/framework/module_guides/models/embeddings/index.md) for more details.
...
[1] https://llamahub.ai/l/readers/llama-index-readers-database
[2] https://llamahub.ai