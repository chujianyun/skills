# developers.llamaindex.ai/python/framework/integrations/vector_stores/pgvectorsdemo/index.md
> https://developers.llamaindex.ai/python/framework/integrations/vector_stores/pgvectorsdemo/index.md

--- title: pgvecto.rs | Developer Documentation ---
...
Then start the pgvecto.rs server as the official document suggests [3]:
...
URL = "postgresql+psycopg://{username}:{password}@{host}:{port}/{db_name}".format( port=os.getenv("DB_PORT", "5432"), host=os.getenv("DB_HOST", "localhost"), username=os.getenv("DB_USER", "postgres"), password=os.getenv("DB_PASS", "mysecretpassword"), db_name=os.getenv("DB_NAME", "postgres"), )
...
from llama_index.core import SimpleDirectoryReader, VectorStoreIndex from llama_index.vector_stores.pgvecto_rs import PGVectoRsStore ```
Download Data
...
vector_store = PGVectoRsStore(client=client) storage_context = StorageContext.from_defaults(vector_store=vector_store) index = VectorStoreIndex.from_documents( documents, storage_context=storage_context ) ```
#### Query Index
...
[3] https://github.com/tensorchord/pgvecto.rs#installation