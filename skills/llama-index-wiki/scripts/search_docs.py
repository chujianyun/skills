#!/usr/bin/env python3
"""Search the bundled LlamaIndex Markdown corpus without third-party packages."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("query", help="English keywords or an exact API/class name")
    parser.add_argument(
        "--prefix",
        default="",
        help="Restrict paths below references/docs, for example module_guides/querying",
    )
...
def score_file(path: Path, docs_root: Path, query: str, terms: list[str], limit: int) -> Match | None:
    ...
    headings = "\n".join(line for line in folded.splitlines() if line.lstrip().startswith("#"))
...
def main() -> int:
    ...
    if docs_root.resolve() not in (search_root, *search_root.parents):
    ...
    if not terms:
        print("Query must contain searchable letters, numbers, dots, underscores, or hyphens", file=sys.stderr)
        return 2
    ...
    if not matches:
        ...
        print("Try an English synonym, API/class name, package name, or a broader --prefix.")
        return 1
